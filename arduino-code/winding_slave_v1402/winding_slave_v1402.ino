...
void receiveEvent(int howMany) { 
  while (1 < Wire.available()) { // loop through all but the last 
    ...
    L = Wire.read(); // receive first byte as an integer (longitude)
    Serial.println(L); // print the integer  
    N = Wire.read(); // receive second byte as an integer (number of layers)
    Serial.println(N); // print the integer
    pasosx = (16 * L / 0.2); // - 10 es un ajuste
    pasos_totales = pasosx*N;
    Serial.println("PASOS TOTALES");
    Serial.println(pasos_totales);
    Serial.println("PASOS X");
    Serial.println(pasosx);
    
    empezar_bobinar = HIGH;
    Serial.println(empezar_bobinar);
  }
